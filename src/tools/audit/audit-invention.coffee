  ...
  @audit: (root_dir) -> (audit_data) -> new Promise (resolve, reject) ->
    try
      ...
      if definitions_with_unknown_categories.length
        console.log " [ERROR] invention definition #{definition.id} has unknown category references #{definition.industryCategoryId}" for definition in definitions_with_unknown_categories
      ...
      if definitions_with_unknown_types.length
        console.log " [ERROR] invention definition #{definition.id} has unknown industry type references #{definition.industryTypeId}" for definition in definitions_with_unknown_types
      ...
      definitions_with_unknown_dependencies = _.filter(definitions, (definition) -> _.find(definition.dependsOnIds, (depend_id) -> !definitions_by_id[depend_id]?)?)
      if definitions_with_unknown_dependencies.length
        console.log " [ERROR] invention definition #{definition.id} has unknown dependency reference #{definition.dependsOnIds}" for definition in definitions_with_unknown_dependencies
